...
func (v RightsFilter) Pre(c Context) (statusCode int, err error) {
    return AirportCloud(c)
}

// 机场云的代码
func AirportCloud(c Context) (statusCode int, err error) {
    // 检查用户是否有权限
    c.GetOriginRequestCtx().QueryArgs().Add("url", string(c.GetOriginRequestCtx().URI().Path()))
    log.Println(string(c.GetOriginRequestCtx().URI().Path()))
    ...
    //pool := thriftserver.GetPool(net.JoinHostPort("jurisdiction.airportcloud", "80"))

    // for local
    pool := thriftserver.GetPool(net.JoinHostPort("127.0.0.1", "8969"))
    ...
    req.Operation = "getRoleButtonAccess"
    req.ParamJSON = paramString

    resp := make(map[string]string)

    r, err := rawClient.Send(req)
    json.Unmarshal(r.ResponseJSON, &resp)

    log.Println(r.ResponeCode.Int())
    log.Println(resp["url"])

    if r.ResponeCode.Int() == 200 && resp["url"] == "true"{
        return
    } else {
        return 701, ErrRightsFailure
    }
}

// 贵宾云的代码
func Guest(c Context) (statusCode int, err error) {
    // 检查用户是否有权限
    c.GetOriginRequestCtx().QueryArgs().Add("url", string(c.GetOriginRequestCtx().URI().Path()))
